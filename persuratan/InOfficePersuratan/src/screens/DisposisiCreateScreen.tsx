...
export default function DisposisiCreateScreen() {
  ...
  const [tanggalDeadline, setTanggalDeadline] = useState(''); // Format: YYYY-MM-DD
  ...
  // Modal visibilities
  const [suratModalVisible, setSuratModalVisible] = useState(false);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        setLoading(true);
        // Fetch active users for picker
        const userRes = await apiClient.get('/users');
        setUserList(userRes.data.data);

        // If surat ID is passed, fetch its detail
        if (paramSuratId) {
          const suratRes = await apiClient.get(`/surat-masuk/${paramSuratId}`);
          setSelectedSurat(suratRes.data.data);
        } else {
          // Fetch letters for picker
          const suratRes = await apiClient.get('/surat-masuk', { params: { per_page: 50 } });
  ...
  const handleSubmit = async () => {
    if (!selectedSurat) {
      Alert.alert('Peringatan', 'Silakan pilih surat masuk rujukan.');
      return;
    }
    if (!isiDisposisi.trim()) {
      Alert.alert('Peringatan', 'Silakan isi instruksi disposisi.');
      return;
    }
    if (selectedPenerima.length === 0) {
      Alert.alert('Peringatan', 'Silakan pilih minimal 1 penerima disposisi.');
      return;
    }
    ...
    try {
      ...
      console.error(error);
      const msg = error.response?.data?.message || 'Gagal mengirim disposisi.';
  ...
  const renderCalendar = () => {
    ...
    const days = [];
    // Spacing for empty days at start of month
  ...
  const filteredSurat = suratList.filter(
    (s) =>
      ...
      s.nomor_surat.toLowerCase().includes(suratSearch.toLowerCase())
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Buat Disposisi Baru</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.formGroup}>
          ...
          {paramSuratId ? (
            <View style={styles.readOnlyInput}>
              <Text style={styles.readOnlyTitle}>{selectedSurat?.perihal}</Text>
              <Text style={styles.readOnlySubtitle}>No: {selectedSurat?.nomor_surat}</Text>
            </View>
          ) : (
            <TouchableOpacity
              style={styles.pickerTrigger}
              onPress={() => setSuratModalVisible(true)}
            >
              <Text style={[styles.pickerTriggerText, selectedSurat && styles.pickerTriggerTextActive]}>
                {selectedSurat ? `${selectedSurat.perihal} (No: ${selectedSurat.nomor_surat})` : 'Pilih Surat Masuk'}
              </Text>
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>Penerima Disposisi (Multi-select)</Text>
          <TouchableOpacity
            style={styles.pickerTrigger}
        ...
        {/* Isi Disposisi Input */}
        <View style={styles.formGroup}>
          <Text style={styles.label}>Instruksi Disposisi</Text>
          <TextInput
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>Tenggat Waktu (Deadline) - Opsional</Text>
          <TouchableOpacity
            style={styles.pickerTrigger}
      ...
      {/* Surat Selection Modal */}
      <Modal visible={suratModalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Pilih Surat Masuk</Text>
              <TouchableOpacity onPress={() => setSuratModalVisible(false)}>
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setSuratModalVisible(false);
                  }}
                >
                  <Text style={styles.listItemTitle}>{item.perihal}</Text>
                  <Text style={styles.listItemSubtitle}>No: {item.nomor_surat} | Dari: {item.pengirim}</Text>
                </TouchableOpacity>
              )}
              ListEmptyComponent={
                <Text style={styles.emptyModalText}>Tidak ada surat masuk yang cocok.</Text>
              }
            />
          </View>
        </View>
